...
def _install_oss_cad_suite(
        target: str,
        user: bool,
        arch: Optional[str] = None,
        os_: Optional[str] = None
    ):
    ...
    if not arch:
        ...
        click.echo(
            click.style("INFO", fg="blue") + f": Auto-detected architecture {arch}."
        )
    ...
    if arch == "i386":
        click.echo(
            ...
            + f": OSS-Cad-Suite does not support `i386` architecture."
        )
        supported = False
    if os_ == "darwin":
        if not arch.lower() in ["arm64", "x64"]:
            click.echo(
                click.style("ERROR", fg="red")
                + f": Darwin only supports arm64 and x64 as architecture."
            )
            supported = False
    if os_ == "windows":
        if not arch.lower() in ["x64"]:
            click.echo(
                click.style("ERROR", fg="red")
                + f": Windows only supports x64 as architecture."
            )
            supported = False
    if not supported:
        click.echo(
            click.style("ERROR", fg="red")
            + f": Please use a different system to build the firmware."
        )
        return
    ...
    with tempfile.TemporaryDirectory() as tempdirname:
        ...
        click.echo(
            ...
            + f": Downloading OSS-CAD-Suite ({os_}/{arch}) ..."
        )
        ...
        click.echo(
            click.style("INFO", fg="blue") + f": Sourcing environment variables ..."
        )
        os.environ["PATH"] += f":{target}/oss-cad-suite/bin"

        # Done!
        click.echo(
            click.style("INFO", fg="blue") + f": OSS-CAD-Suite successfully installed"
        )
...
def _install_openocd_rpi(target: str, user: bool):
    ...
    click.echo(
        click.style("INFO", fg="blue") + f": Installing OpenOCD for Raspberry Pi."
    )
    ...
    if subprocess.call("sudo apt-get update", shell=True):
    ...
    with tempfile.TemporaryDirectory() as tempdirname:
        ...
        if subprocess.call(
            "git clone http://openocd.zylin.com/openocd", cwd=tempdirname, shell=True
        ):
        ...
        click.echo(
            click.style("INFO", fg="blue")
            + f": Configuring and making OpenOCD (this make take a while) ..."
        )
        command = f"./bootstrap && "
        command += (
            "./configure --enable-sysfsgpio --enable-bcm2835gpio --enable-libgpio && "
        )
        command += "make"
        if subprocess.call(
            command, cwd=os.path.join(tempdirname, "openocd"), shell=True
        ):
        ...
        if subprocess.call(
            "sudo make install", cwd=os.path.join(tempdirname, "openocd"), shell=True
        ):
...
def cli(user, directory, arch, os_):
    ...
    if (not os_ and platform.system().lower() == "linux") or os_ == "linux":
        if subprocess.call("sudo apt-get -y install openocd", shell=True):
            click.echo(
                click.style("ERROR", fg="red")
                + ": Cannot install OpenOCD (sudo apt-get install failed)."
            )
            return -1
